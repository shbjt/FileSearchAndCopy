...
class FileSearchAndCopy:
    ...
    def create_widgets(self):
        ...
        ttk.Radiobutton(self.root, text="精确匹配", variable=self.search_option, value="exact").grid(row=4, column=1, padx=5, pady=5, sticky="w")
        ttk.Radiobutton(self.root, text="包含匹配", variable=self.search_option, value="contains").grid(row=4, column=2, padx=5, pady=5, sticky="w")
        
        # 文件扩展名选项
        self.extension_option = tk.StringVar(value="any")
        ttk.Label(self.root, text="文件扩展名:").grid(row=5, column=0, padx=5, pady=5, sticky="w")
        ttk.Radiobutton(self.root, text="任何扩展名", variable=self.extension_option, value="any").grid(row=5, column=1, padx=5, pady=5, sticky="w")
        ttk.Radiobutton(self.root, text="无扩展名", variable=self.extension_option, value="none").grid(row=5, column=2, padx=5, pady=5, sticky="w")
        
        # 进度条
        self.progress = ttk.Progressbar(self.root, orient=tk.HORIZONTAL, length=400, mode='determinate')
        self.progress.grid(row=6, column=0, columnspan=3, padx=5, pady=10, sticky="ew")
        
        # 按钮
        ttk.Button(self.root, text="开始搜索并复制", command=self.start_process).grid(row=7, column=1, padx=5, pady=10)
        
        # 结果文本框
        ttk.Label(self.root, text="操作结果:").grid(row=8, column=0, padx=5, pady=5, sticky="w")
        self.result_text = tk.Text(self.root, height=15, width=80)
        self.result_text.grid(row=9, column=0, columnspan=3, padx=5, pady=5, sticky="nsew")
    ...
    def parse_file_list(self):
        ...
        try:
            ...
            # 改进的正则表达式，能够识别1-14位数字序列
            pattern = r'\b\d{1,14}(?:\.\w{1,5})?\b'
    ...
    def search_files(self, file_names):
        ...
        extension_option = self.extension_option.get()
        ...
        for root_dir, _, files in os.walk(source_dir):
            for file in files:
                ...
                # 根据扩展名选项过滤文件
                if extension_option == "none" and "." in file:
                    continue
                ...
                if search_option == "exact":
                    ...
                    # 也检查不带扩展名的匹配
                    elif "." in file and file.split(".")[0] in file_names:
                        matched_files.append(file_path)
                else:
                    # 包含匹配：文件名包含列表中的某个名称
